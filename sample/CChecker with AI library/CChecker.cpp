...
CChecker::CChecker(const string& game_state) {
...
string CChecker::getGameStateString() const {
	string ret;	
	for(int i=0;i<size_;i++) {
		for(int j=0;j<size_;j++) {
...
CChecker CChecker::parseGameStateString(const string& game_state) {
	return CChecker(game_state);	
}

CChecker CChecker::simulate(GameMove move) const {
	CChecker ret = *this;
...
int CChecker::getSize() const {
	return size_;
}

void CChecker::removePiece(Point p) {
	board_[p.row][p.col].is_there_piece = false;
}

void CChecker::setPiece(Point p, int player_number, bool is_king) {
	board_[p.row][p.col].is_there_piece = true;
	board_[p.row][p.col].player_number = player_number;
	board_[p.row][p.col].is_king = is_king;
}

bool CChecker::isMoveValid(GameMove move) const {	
...
bool CChecker::movePiece(GameMove move) {		
	if(!isMoveValid(move)) 
		return false;
...
vector<Point> CChecker::getWalkableFromCoinInTile(int row, int col) const {
...
vector<GameMove> CChecker::getAllLegalMoves() const {
...
void CChecker::printBoard() const{
	cout<<"    ";
	for(int i=0;i<size_;i++)
		cout<<" "<<i<<" ";
	cout<<endl;
...
GameTile CChecker::convertToTile(char c) {
	int v = c - '0';
	GameTile ret;
	if(v == 0) {
...
bool CChecker::isThereEatable() const {
	for(int i=0;i<size_;i++) {
		for(int j=0;j<size_;j++) {			
			if(!board_[i][j].is_there_piece || board_[i][j].player_number != turn_) 
				continue;			
...
int CChecker::getTurn() const {
	return turn_;
}

int CChecker::whoWin() const {
	return win_;
}