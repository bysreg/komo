...
function monteCarloExpand(child, move_index, parent, profile)	
	...
	if(profile.max_tree_size ~= nil and profile._mc.size > profile.max_tree_size) then
		--ran out of memory, time to free some
		monteCarloSweep(profile)
	end
end

function monteCarloSweep(profile)
	-- simple way to clear some memories
	--fixme
end
